...
int main()
{
    ...
    double L = 32.0; //Tamaño del lado de la caja cuadrada
    int N = 160; //Número de átomos dentro de la caja
    ...
    for (int k = 0; k < N; k++) 
    {
        v_x[k] = 3*cos(angulos[k]);
        v_y[k] = 3*sin(angulos[k]);
    }
...
void algoritmo_verlet(vector < double > &x, vector < double > &y, vector < double > &v_x, vector < double > &v_y, double L, int N) 
{
    ...
    ofstream archivo_velocidades("Velocidades_atomos_v_3_2.txt");
    ...
    // Genero un array para la presión que ejercen las partículas en las paredes de la caja
    //En cada paso temporal, la presión se calcula como el momento transferido a la caja
    //entre la longitud de la caja y el tiempo transcurrido
  
    vector < double > Presion(P, 0.0);
...
// Se ha de tener en cuenta las condiciones periódicas al calcular las nuevas posiciones de los átomos
        ...
        Presion[k-1] = momento / (4 * L * t); // El área de la caja es 4 * L porque cada pared tiene una longitud L y hay 4 paredes
    ...
    ofstream archivo_presion("Presion_v_3_2.txt");

    for (int k = 0; k < P; k++) 
    {
        archivo_presion << Presion[k] << '\n';
    }
