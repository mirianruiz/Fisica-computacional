...
void algoritmo_verlet(vector < double > &x, vector < double > &y, vector < double > &v_x, vector < double > &v_y, double L, int N) 
{
    ...
    ofstream archivo_posiciones("Posiciones_atomos_ej_6.txt");
    ofstream archivo_velocidades("Velocidades_atomos_ej_6.txt");
    ofstream archivo_posicion_atomo_sin_c_c("Posicion_Atomo_sin_condiciones_periodicas_ej_6.txt");
    ...
    for (int i = 0; i < N; i++) 
    {
        ...
        archivo_posicion_atomo_sin_c_c << x_sin_c_c[i] << ' ' << y_sin_c_c[i] << ' ';
    }
    archivo_posiciones << '\n';
    archivo_velocidades << '\n';
    archivo_posicion_atomo_sin_c_c << '\n';

    
   
    //Quiero evolucionar el sistema hasta t=500, si el paso temporal es h=0.002,
    ...
    for (int k = 1; k <= P; k++) 
    {
       //************************paso 2. Calculo wi  y la posición en t+h********************** """
        ...
        for (int i = 0; i < N; i++) 
        {
            ...
            //Tratectoria real de los átomos sin aplicar condiciones periódicas
        ...
        // Reescalo las velocidades para aumentar la temperatura en t = 180, 270, 300 y 400 segundos
        if ((k == 90000) || (k == 135000) || (k == 150000) || (k == 200000))
        {
            for (int i = 0; i < N; i++) 
            {
                v_x[i] *= 1.5;
    ...
    archivo_posicion_atomo_sin_c_c.close();
}
