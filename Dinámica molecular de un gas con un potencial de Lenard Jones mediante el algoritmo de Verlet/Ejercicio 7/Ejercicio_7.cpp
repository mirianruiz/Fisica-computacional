...
int main()
{
    ...
    double L = 12.0; //Tamaño del lado de la caja cuadrada
...
void algoritmo_verlet(vector < double > &x, vector < double > &y, vector < double > &v_x, vector < double > &v_y, double L, int N) 
{
    ...
    //Guardo el vector de la separación cuadrática media en un fichero
    ofstream archivo_separacion("Separacion_cuadratica_media_entre_particulas.txt");

    // Genero un array para guardar las posiciónes del los átomos sin aplicar las condiciones periódicas 
    vector<double> x_sin_c_c = x;
    vector<double> y_sin_c_c = y;
    ...
    //Quiero evolucionar el sistema hasta t=3500, si el paso temporal es h=0.002,
    //el número de pasos temporales 
    int P = int(3500 / h);

    // Vctor para guardar la separación media de dos átomos en cada instante temporal
    vector<double> separacion(P);
    ...
    for (int k = 1; k <= P; k++) 
    {
       //************************paso 2. Calculo wi  y la posición en t+h********************** """
        ...
        for (int i = 0; i < N; i++) 
        {
            ...
            //Trayectoria real de los átomos sin aplicar condiciones periódicas
            x_sin_c_c[i] = x_sin_c_c[i] + h * w_x[i];
            y_sin_c_c[i] = y_sin_c_c[i] + h * w_y[i];
        }
        ...
        for (int i = 0; i < N; i++) 
        {
            v_x[i] = w_x[i] + (h / 2.0) * a_x[i];
            v_y[i] = w_y[i] + (h / 2.0) * a_y[i];
        }
        ...
        //CALCULAMOS LA SEPARACIÓN MEDIA EN UN INSTANTE Y LA GUARDO EN EL VECTOR
        double suma_sep_cuadratica = 0.0;
        int num_pares = 0;

        for (int i = 0; i < N; i++) {
            for (int j = i + 1; j < N; j++) {
                double dx = x_sin_c_c[i] - x_sin_c_c[j];
                double dy = y_sin_c_c[i] - y_sin_c_c[j];
                suma_sep_cuadratica += (dx * dx + dy * dy);
                num_pares++;
            }
        }
        
        // GuardO en la posición k-1 (porque los vectores empiezan en 0 y k empieza en 1)
        separacion[k - 1] = suma_sep_cuadratica / num_pares;

        archivo_separacion << separacion[k - 1] << endl;

        // Reescalo las velocidades para aumentar la temperatura en ciertos pasos temporales
