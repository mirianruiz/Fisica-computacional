...
void algoritmo_verlet_modificado(vector < double > &x, vector < double > &y, vector < double > &v_x, vector < double > &v_y, double L, int N, int num_bins, double r_max, double r_min, double ancho_bin) 
{
    ...
    vector < int > h_solido(num_bins, 0); 
    vector < int > h_liquido(num_bins, 0); 
    ...
    for (int k = 1; k <= P; k++) 
    {
       //************************paso 2. Calculo wi  y la posición en t+h********************** """
        ...
        // Reescalo las velocidades para aumentar la temperatura en t = 500, 1000, 2000  
        ...
        if ((k >= 200000) && (k <= 500000) && (k % 100 == 0))
        {
            histograma(x, y, L, N, r_max, r_min, ancho_bin, h_solido);
        }

        // Guardo en el histograma de líquido a partir de t = 1600 

        if ((k >= 1600000) && (k % 100 == 0)) 
        {
            histograma(x, y, L, N, r_max, r_min, ancho_bin, h_liquido);
        }
    ...
    {
        archivo_histograma_solido << h_solido[i] << '\n';
        archivo_histograma_liquido << h_liquido[i] << '\n';
    }
